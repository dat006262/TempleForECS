...
namespace Unity.Physics.Stateful
{
    ...
    public partial struct StatefulCollisionEventSystem : ISystem
    {
        ...
        {
            events = new StatefulSimulationEventBuffers<StatefulCollisionEvent>();//cai j do
            events.AllocateBuffers();  //lam j do
        ...
        {
            new ClearCollisionEventDynamicBufferJob().ScheduleParallel();          //ClearBuffer

            events.SwapBuffers();                               //???

            var currentEvents = events.Current;                     //Va cham bay h

            var previousEvents = events.Previous;                       //Va cham truoc do

            state.Dependency = new CollectCollisionEventsWithDetailsJob
            {
                CollisionEvents = currentEvents,    //va cham bay h
            ...
            {
                CurrentEvents = currentEvents,          //va cham bay h
                PreviousEvents = previousEvents,        //va cham truoc do
